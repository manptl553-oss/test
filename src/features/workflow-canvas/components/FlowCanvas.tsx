...
export function mapWorkflowToFlow(workflow: any, actions?: any) {
  ...
  // 1. Map Nodes
  workflow.nodes?.forEach((wfNode: any) => {
    let outputs: string[] = [];
    ...
    switch (wfNode.type) {
      ...
      case "rule_executor":
        ...
        break;
      case "switch":
        outputs = wfNode.config?.switchCases?.map((c: any) => c.condition) || [];
        break;
      case "loop":
    ...
    nodes.push({
      ...
      data: {
        ...
        parentLoop: wfNode.parentId ?? null,
        outputs,
        ...actions,
      },
  ...
  workflow.edges?.forEach((e: any) => {
    ...
    const sourceNode = workflow.nodes.find((n: any) => n.id === e.sourceId);
    ...
    edges.push({
      ...
      label: (e.condition && e.condition !== "none" && e.condition !== "body") ? e.condition : "",
