...
export function NodeConfigModal({ open, onOpenChange, nodeId, nodeData }: any) {
  ...
  const defaultValues = useMemo(() => {
    ...
    const saved = nodeData?.configuration?.[isTrigger ? nodeType : undefined] ?? nodeData?.configuration ?? {};
    const result: any = {}; fields.forEach((f: any) => { const val = saved[f.name]; result[f.name] = val ?? ''; });
    return result;
  }, [nodeData, nodeType]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[750px] bg-white rounded-xl shadow-xl" onOpenAutoFocus={(e: any) => e.preventDefault()}>
        <DialogHeader className="border-b border-gray-200 pb-4">
          <DialogTitle className="font-semibold text-lg text-gray-800">
            <input value={nodeName} onChange={(e) => setNodeName(e.target.value)} className="outline-none" />
          </DialogTitle>
        </DialogHeader>
        {isFormNode && <DynamicForm key={nodeId} fields={fields as any} defaultValues={defaultValues} onSubmit={handleFormSubmit} schema={schema as any} onClose={onOpenChange} />}
        {!isFormNode && <div className="p-4 text-sm text-gray-600">Custom UI for conditional/switch is not included in this package build.</div>}
      </DialogContent>
    </Dialog>
  );
}
